...
```mermaid
graph TD
    subgraph orderer_node ["Servicio de Ordenación"]
        Orderer["orderer.example.com<br/>Puerto: 7050"]
    end

    subgraph org1_node ["Org1MSP: Institución Educativa"]
    ...
    subgraph org2_node ["Org2MSP: Empresa Verificadora"]
...
Crea un directorio para almacenar los archivos del chaincode dentro de la máquina:
...
Crea el archivo `package.json` en la misma ubicación que el contrato:
...
Asegúrate de estar en el directorio raíz de la red de CA (`$HOME/red-con-ca`) antes de ejecutar los siguientes comandos en la terminal.
...
Primero, obtenemos el Package ID que nos ha generado Fabric al instalar:
```bash
peer lifecycle chaincode queryinstalled
```
*Guarda el hash devuelto (lo que aparece después de "Package ID:") en una variable de entorno:*
```bash
export CC_PACKAGE_ID=certiledger_1.0:PEGAR_AQUI_EL_HASH_EXACTO
```

**Aprobar como Org2 (Empresa):**
```bash
...
Ejecuta estos pasos para demostrar el funcionamiento del control de accesos (RBAC).
...
Cambiamos las variables a la identidad de Org2:
```bash
export CORE_PEER_LOCALMSPID=Org2MSP
export CORE_PEER_ADDRESS=localhost:9051
export CORE_PEER_TLS_ROOTCERT_FILE=$PEER_ORG2_TLS
